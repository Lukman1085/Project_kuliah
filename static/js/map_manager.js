...
export const mapManager = { 
    ...
    // [BARU] Menyimpan GeoJSON Features di memori agar bisa diupdate propertinya
    _currentFeatures: [], 
    ...
    // ... (setActiveMarkerHighlight, removeActiveMarkerHighlight, resetActiveLocationState TETAP SAMA) ...
    ...
    perbaruiPetaGeo: async function() {
            ...
            if (zoom <= 7.99) { 
                ...
                this._currentFeatures = []; // Reset fitur lokal
                try {
                ...
                try {
                    ...
                    const features = geoOnly.map(g => ({ 
                        ...
                        properties: { 
                            ...
                            type: 'kabkec',
                            tipadm: g.tipadm,
                            icon_name: 'wi-na' // Default awal
                        } 
                    }));
                    
                    // [BARU] Simpan ke variabel lokal dan update source
    ...
    // [FUNGSI BARU] Update Ikon di Source (Layout Property Workaround)
    updateIconsForTime: function(idxGlobal) {
        const map = this.getMap();
        if (!map || !this._currentFeatures || this._currentFeatures.length === 0) return;

        let hasChanges = false;
        const source = map.getSource('data-cuaca-source');

        // Loop semua fitur yang ada di peta saat ini
        this._currentFeatures.forEach(feature => {
            const id = feature.id;
            const cachedData = cacheManager.get(id);
            
            if (cachedData && cachedData.hourly?.time && idxGlobal < cachedData.hourly.time.length) {
                // Ambil kode cuaca dan is_day
                const rawCode = cachedData.hourly.weather_code?.[idxGlobal];
                const rawIsDay = cachedData.hourly.is_day?.[idxGlobal];
                
                // Hitung nama ikon
                const weatherInfo = utils.getWeatherInfo(rawCode, rawIsDay);
                const newIcon = weatherInfo.raw_icon_name;

                // Update property jika berbeda
                if (feature.properties.icon_name !== newIcon) {
                    feature.properties.icon_name = newIcon;
                    hasChanges = true;
                }
            }
        });

        // Jika ada perubahan ikon, refresh source (ini agak berat, tapi satu-satunya cara untuk icon-image layout)
        if (hasChanges && source) {
            source.setData({ type: 'FeatureCollection', features: this._currentFeatures });
        }
    },

    // ... (fetchDataForVisibleMarkers, _processIncomingData TETAP SAMA) ...
    fetchDataForVisibleMarkers: async function() {
        ...
        try {
            ...
            for (const id in dataMap) {
                ...
                if (!currentSourceIds.has(String(id))) { this._processIncomingData(id, data); continue; }
            ...
            if (!isFirstLoad) { timeManager.updateMapFeaturesForTime(timeManager.getSelectedTimeIndex()); }
        }
    }, 
    ...
    // Modifikasi: Hapus injection 'icon_name' ke Feature State (karena sudah dipindah ke Properties/setData)
    _updateMapStateForFeature: function(id, data, isActive) {
        ...
        if (data?.hourly?.time && idxSaatIni >= 0 && idxSaatIni < data.hourly.time.length) {
            ...
            try {
                map.setFeatureState(
                    { source: 'data-cuaca-source', id: id },
                    {
                        ...
                        // HANYA WARNA DI SINI (Paint Property)
            ...
            try { map.setFeatureState({ source: 'data-cuaca-source', id: id }, { hasData: false, active: isActive }); } catch (e) { }
        }
    },

    // ... (handleClusterClick, handleProvinceClick, handleUnclusteredClick, _handleInflightState, _handleCacheHit, _handleCacheMiss, _renderRichPopup TETAP SAMA) ...
    handleClusterClick: async function(feature, coordinates) { 
        const map = this.getMap(); if (!map) return;
        const protocol = window.location.protocol; const hostname = window.location.hostname; const port = '5000'; const baseUrl = `${protocol}//${hostname}:${port}`;
        popupManager.close(true); const clusterId = feature.properties.cluster_id; const source = map.getSource('data-cuaca-source'); 
    ...
    handleUnclusteredClick: function(props) {
        ...
        const coordinates = [lon, lat]; if (!coordinates || isNaN(coordinates[0]) || isNaN(coordinates[1])) { return; }
        console.log(`Handling Click: ${nama_simpel} (${id}, TIPADM: ${tipadm})`); 
        popupManager.close(true);
        const previousId = this._activeLocationId;
        if (previousId && previousId !== id) { this.removeActiveMarkerHighlight(previousId, true); }
        this._activeLocationId = id; this._activeLocationSimpleName = nama_simpel; this._activeLocationLabel = nama_label; this._previousActiveLocationId = previousId;
    ...
    _handleCacheHit: function(props, data, coordinates) {
        console.log(`Cache hit for ${props.id}.`); if (!data.nama_simpel) data.nama_simpel = props.nama_simpel; if (!data.nama_label) data.nama_label = props.nama_label;
    ...
    _handleCacheMiss: async function(props, coordinates) {
        ...
        try {
            ...
            if (this._activeLocationId === id) {
                ...
                this._renderRichPopup(data, coordinates);
                if (sidebarManager.isOpen()) sidebarManager.renderSidebarContent();
            } else { this._updateMapStateForFeature(id, data, false); }
        } catch (e) { 
